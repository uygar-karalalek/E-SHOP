package com.uygar.eshop.persistence.entities.mapper

import com.uygar.eshop.persistence.entities.ShoppingCard
import com.uygar.eshop.core.ShoppingCard as ShoppingCardDomain

object ShoppingCardMapper {

    fun mapToEntity(shoppingCard: ShoppingCardDomain): ShoppingCard {
        return ShoppingCard(
            shoppingCard.id,
            UserMapper.mapToDto(shoppingCard.user),
            shoppingCard.cardItems.map(CardItemMapper::mapToEntity)
        )
    }

    fun mapToDomain(shoppingCar: ShoppingCard): ShoppingCardDomain {
        return ShoppingCardDomain(
            shoppingCar.id,
            UserMapper.mapToDomain(shoppingCar.user),
            shoppingCar.cardItems.map(CardItemMapper::mapToDomain)
        )
    }

}